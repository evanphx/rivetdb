...
func TestMerge(t *testing.T) {
	...
	n.It("discards older, conflicted values", func() {
		path := filepath.Join(tmpdir, "one")
		defer os.Remove(path)
		...
		v, err = r.Get(2, []byte("4"))
		require.NoError(t, err)

		v, err = r.Get(2, []byte("5"))
		require.NoError(t, err)
